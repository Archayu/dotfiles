...
# Modified to auto-read hex from ~/.config/matugen/papirus/papirus.txt,
# pick the closest color (Catppuccin Mocha mapping) and apply it to Papirus-Dark.
#
# @author: Sergei Eremenko (https://github.com/SmartFinn)
# @modified-by: you (with ChatGPT)
...
# --- new helper: choose closest color from papirus.txt and mapping ---
choose_closest_color_from_file() {
	# path to user's color file (change if you store it elsewhere)
	local papirus_hex_file="${XDG_CONFIG_HOME:-$USER_HOME/.config}/matugen/papirus/papirus.txt"
	local hex input_r input_g input_b
	local name r g b distance min_distance=999999999 closest=''
	local -A COLOR_HEX

	# catppuccin mocha mapping (same you used before)
	COLOR_HEX=(
	...
	if [ ! -f "$papirus_hex_file" ]; then
		fatal "Hex file not found: $papirus_hex_file"
	fi

	hex="$(tr -d '[:space:]' < "$papirus_hex_file")"
	[ -n "$hex" ] || fatal "Hex file is empty: $papirus_hex_file"

	# sanitize leading '#'
	hex="${hex#"#"}"
	if [[ ! "$hex" =~ ^[0-9A-Fa-f]{6}$ ]]; then
		fatal "Invalid HEX value in $papirus_hex_file: $hex"
	fi
	...
	read input_r input_g input_b <<< "$(hex_to_rgb "$hex")"
	verbose "Input HEX: #$hex -> RGB: $input_r $input_g $input_b"

	# Get available colors from theme (declare_colors depends on THEME_DIR)
	eval "$(declare_colors)" || {
		fatal "Unable to list available colors from theme '$THEME_NAME'"
	}

	# If declare_colors returned nothing, fatal
	[ "${#colors[@]}" -gt 0 ] || fatal "No color variants found in '$THEME_DIR'"

	# Compare only colors that are present in both theme and our mapping
	min_distance=$min_distance

	for name in "${colors[@]}"; do
		# only compare mapped colors
		if [ -z "${COLOR_HEX[$name]+_}" ]; then
			verbose "Skipping unmapped color '$name'"
			continue
		fi

		read r g b <<< "$(hex_to_rgb "${COLOR_HEX[$name]#"#"}")"

		distance=$(( (input_r - r) * (input_r - r) + (input_g - g) * (input_g - g) + (input_b - b) * (input_b - b) ))
		verbose "Distance to $name (${COLOR_HEX[$name]}): $distance"
	...
	# if nothing matched (no mapped colors present in theme), fallback to first declared color
	if [ -z "$closest" ]; then
		closest="${colors[0]}"
		verbose "No mapped colors found in theme; falling back to first available color: $closest"
	fi

	SELECTED_COLOR="$closest"
	msg "Detected color #$hex -> closest theme color: $SELECTED_COLOR"
}

main() {
	# default values of options
	declare THEME_NAME="${THEME_NAME:-Papirus-Dark}"   # Force dark only
	...
	# allow -h etc to still work, but if no args given we proceed with auto behavior
	if [ "$#" -gt 0 ]; then
		parse_args "${ARGS[@]}"
	fi
	...
	# Force theme to Papirus-Dark (dark mode only)
	# If user explicitly passed -t, parse_args would have changed THEME_NAME; we still override
	...
	# Choose color automatically from papirus.txt and available colors
	choose_closest_color_from_file
